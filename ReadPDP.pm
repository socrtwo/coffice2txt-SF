...
package Spreadsheet::ReadPDP;

=head1 NAME

 Spreadsheet::ReadPDP - Read the data from a spreadsheet
...
# Spreadsheet::ReadPDP::parses ("csv") or die "Cannot parse CSV"
sub parses
{
...
sub ReadData
{
	...
	my @data = ( {
	    ...
	    version	=> $parse_type eq "XLSX"
			 ? $Spreadsheet::XLSX::ZipMod2::VERSION
...
Spreadsheet::ReadPDP tries to transparently read *any* spreadsheet and
...
=item my @rows = Spreadsheet::ReadPDP::rows ($ss->[1])
...
=item Spreadsheet::ReadPDP::parses ("CSV")

C<parses ()> returns Spreadsheet::ReadPDP's capability to parse the
required format.
...
=item my $v = Spreadsheet::ReadPDP::Version ()

Returns the current version of Spreadsheet::ReadPDP.
...
New Spreadsheet::ReadPDP options are bound to happen. I'm thinking of an
...
this format to be supported in Spreadsheet::ReadPDP are hereby motivated to
